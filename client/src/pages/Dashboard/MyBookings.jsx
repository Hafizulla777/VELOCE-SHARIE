...
import api from '../../utils/api';
import Loader from '../../components/Loader/Loader';
import BookingCard from '../../components/Dashboard/BookingCard'; // Reusing the fixed component above
...
const STATUS_FILTERS = [
  ...
  { value: 'approved', label: 'Confirmed' }, // Mapped to backend 'approved'
...
const MyBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        // FIX: Removed 'simple'. Applied double-nesting rule.
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="visible">
        ...
        <p className="text-sm text-white/35 mt-1">View and manage all your rental reservations</p>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2 text-white/40 text-xs font-semibold uppercase tracking-wider mr-2">
        ...
        {STATUS_FILTERS.map((filter) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-300 border ${activeFilter === filter.value
                ? 'bg-primary-500/15 text-primary-400 border-primary-500/20'
                : 'bg-white/[0.02] text-white/50 border-white/[0.06] hover:text-white hover:border-white/10'
              }`}
          >
            {filter.label}
          </button>
      ...
      {filteredBookings.length === 0 ? (
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="rounded-2xl border border-dashed border-white/10 bg-white/[0.01] p-16 text-center"
        >
